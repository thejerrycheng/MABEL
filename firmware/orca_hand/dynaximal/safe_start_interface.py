...
MIN_POS             = 0    
MAX_POS             = 4095 
MAX_TORQUE_PWM      = 885  # RESTORED TO FULL POWER (Heavy Lifting)

# THE VIRTUAL SPRING TENSION
# This is how far the target is allowed to get "ahead" of the physical motor.
# 150 creates a very strong, firm grip without triggering thermal overload.
MAX_LEAD            = 150  
...
def main():
    ...
    # Initialize both Write AND Read Sync modules
    ...
    for dxl_id in range(1, 17):
        ...
        if dxl_comm_result == COMM_SUCCESS:
            ...
            # Add each found motor to the high-speed SyncRead parameter list
    ...
    try:
        while True:
            try:
                # --- A. READ SENSORS (Where the magic happens) ---
                actual_positions = {}
                ...
                # This is what stops the Overload Error while allowing max torque
                if actual_positions:
                    for dxl_id in active_ids:
                        actual_pos = actual_positions[dxl_id]
                        target_pos = targets[dxl_id]
                        
                        # If the target gets too far ahead of reality (meaning the finger is stalled against an object)
